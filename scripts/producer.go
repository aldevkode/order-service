...
type Msg struct{ Raw json.RawMessage }

func main(){
    w := &kgo.Writer{Addr: kgo.TCP("localhost:9092"), Topic: "orders"}
    defer w.Close()
    ...
    if err := w.WriteMessages(context.Background(), kgo.Message{Value: sample, Time: time.Now()}); err != nil {
        log.Fatal(err)
    }
    log.Println("sent sample order")
}